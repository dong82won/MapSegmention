...
int main() {
    ...
    for (size_t i = 0; i < circlesCenters.size(); )
    {
        ...
        if ( db.feturePoints.size() < 2)  {  
            ...
            // B 데이터와 최소 거리 저장을 위한 벡터
            std::vector<std::pair<cv::Point, double>> pointsBWithDistances;
            std::cout <<std::endl;
            std::cout << "FeaturePt:\n";
            for (const auto &pt : db.feturePoints)
            {
                double minDist = findMinDistanceToContour(pt, db.trajectoryPoints);
                pointsBWithDistances.push_back(std::make_pair(pt, minDist));
            ...
            // 최소 거리 기준으로 정렬
            std::sort(pointsBWithDistances.begin(), pointsBWithDistances.end(),
                      [](const std::pair<cv::Point, double> &a, const std::pair<cv::Point, double> &b)
                      {
                          return a.second < b.second;
                      });

            // 정렬된 Feature Points 데이터와 거리 출력
            std::cout << "Sorted Feature Points by distance to Contour (with distances):" << std::endl;
            for (const auto &entry : pointsBWithDistances)
            {
                std::cout << "Point: (" << entry.first.x << ", " << entry.first.y << ") - Distance: " << entry.second << std::endl;
            }
            //---------------------------------------------------------------------------------------------------------
