...
Mat rotateImage(const Mat &image, double angle)
{
    ...
    rotationMatrix.at<double>(0, 2) += (new_width / 2.0) - center.x;
    rotationMatrix.at<double>(1, 2) += (new_height / 2.0) - center.y;


    Mat rotatedImage;
    warpAffine(image, rotatedImage, rotationMatrix, Size(new_width, new_height));
    return rotatedImage;
}
