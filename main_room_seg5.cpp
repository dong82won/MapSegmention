...
int main() {
    ...
    for (size_t i = 0; i < circlesCenters.size(); )
    {
        ...
        cv::drawMarker(result_img, cp, cv::Scalar(0, 0, 255), cv::MARKER_CROSS);
        ...
        if ( db.feturePoints.size() < 2)  { 
            ...
            // std::vector를 cv::Mat로 변환
            cv::Mat dataMat(db.feturePoints.size(), 2, CV_32F);
            for (size_t p = 0; p < db.feturePoints.size(); p++)
            {
                dataMat.at<float>(p, 0) = static_cast<float>(db.feturePoints[p].x);
                dataMat.at<float>(p, 1) = static_cast<float>(db.feturePoints[p].y);
            }

            // K-Means 알고리즘 실행
            cv::kmeans(dataMat, k, labels, cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 100, 0.2), 10, cv::KMEANS_PP_CENTERS, centers);
 
            // 결과 출력
            std::cout<< std::endl;
            for (int p = 0; p < dataMat.rows; p++)
            {
                std::cout << "Point: (" << dataMat.at<float>(p, 0) << ", " << dataMat.at<float>(p, 1) << ") - Cluster: " << labels.at<int>(p) << std::endl;
                
                if (labels.at<int>(p) == 0) 
                    db.addfetureCluster0Pts(cv::Point((int)dataMat.at<float>(p, 0), (int)dataMat.at<float>(p, 1))); 
                else
                    db.addfetureCluster1Pts(cv::Point((int)dataMat.at<float>(p, 0), (int)dataMat.at<float>(p, 1)));  
            }

            // 클러스터 중심 출력
            std::cout << "Cluster Centers:" << std::endl;
            for (int p = 0; p < centers.rows; p++)
            {
                std::cout << "Center " << p << ": (" << centers.at<float>(p, 0) << ", " << centers.at<float>(p, 1) << ")" << std::endl; 
            }
            db.addCluster0Center(cv::Point2f( centers.at<float>(0, 0), centers.at<float>(0, 1)));
            db.addCluster1Center(cv::Point2f( centers.at<float>(1, 0), centers.at<float>(1, 1)));
    ...
    cv::imshow("result_img0", result_img);

    cv::waitKey(0);


    cv::namedWindow("result_img2", WINDOW_KEEPRATIO && WINDOW_AUTOSIZE); 
    for (size_t i = 0; i < database.size(); i++)
    {

        cv:Mat img_update =  result_img2.clone();

        cv::Point cp  =database[i].centerPoint; 
        drawingSetpRectangle(img_update, cp, radius);

        std::cout << "TrajectoryPts:";
        for (const auto& pt : database[i].trajectoryPoints) 
        { 
            std::cout << "(" << pt.x << ", " << pt.y << ") ";
        ...
        for (size_t j = 0; j < database[i].fetureCluster0.size(); j++)
        {
            std::cout << "( " << database[i].fetureCluster0[j].x << ", " << database[i].fetureCluster0[j].y << ")";
            cv::circle(img_update, database[i].fetureCluster0[j], 3, CV_RGB(255, 0, 0), -1);
        }

        std::cout << std::endl;
        std::cout << "Cluster1:" << std::endl;
        for (size_t j = 0; j < database[i].fetureCluster1.size(); j++)
        {
            std::cout << "( " << database[i].fetureCluster1[j].x << ", " << database[i].fetureCluster1[j].y << ")";
            cv::circle(img_update, database[i].fetureCluster1[j], 3, CV_RGB(0, 255, 0), -1);
        }
        std::cout << std::endl;
        std::cout <<"======================================================================="<< std::endl;
        
        cv::imshow("result_img2", img_update); 
        cv::waitKey();
