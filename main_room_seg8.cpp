...
// 함수: 이미지 회전
double rotateImageAngle(const Mat& image)
{
...
std::vector<cv::Point> findTurningPoints(const std::vector<cv::Point> &coords, int windowSize)
{
    ...
    for (size_t i = windowSize; i < coords.size() - windowSize; ++i)
    {
        ...
        {
            double slope = (coords[i].x - coords[i - j].x != 0) ? static_cast<double>(coords[i].y - coords[i - j].y) / (coords[i].x - coords[i - j].x) : std::numeric_limits<double>::infinity();
            avgSlope1 += slope;
        }
        ...
        for (int j = 1; j <= windowSize; ++j)
        {
            double slope = (coords[i + j].x - coords[i].x != 0) ? static_cast<double>(coords[i + j].y - coords[i].y) / (coords[i + j].x - coords[i].x) : std::numeric_limits<double>::infinity();
            avgSlope2 += slope;
        }
...
int main()
{
    ...
    cv::flip(raw_img, raw_img, 0); // 1은 좌우 반전 
    ...
    for (size_t i = 0; i < circlesCenters.size();)
    {
        ...
        else
        {
            ...
            std::vector<LINEINFO> line = checkPointsNearLineSegments(db.feturePoints, db.trajectoryPoints, 3);
            db.lines = line;
    ...
    std::vector<LINEINFO> result;
    cv::namedWindow("test", WINDOW_KEEPRATIO && WINDOW_AUTOSIZE);
    for (size_t i=0; i<database.size(); i++)
    {
        cv::Mat img_update = result_img.clone();
        std::cout <<"----------------------------------------------------------" <<std::endl;

        // std::pair<cv::Point, cv::Point> linetmp;
        // std::vector<std::pair<cv::Point, cv::Point>> resultline;

        LINEINFO temp; 
        for (size_t j =0; j < database[i].lines.size(); j++)
        {
            cv::Point startpt = database[i].lines[j].virtual_wll.first;
            cv::Point endpt = database[i].lines[j].virtual_wll.second;
            double distance = database[i].lines[j].distance; 
            cv::Point center = database[i].centerPoint;
 

            drawingSetpRectangle(img_update, center, radius); 
            cv::drawMarker(img_update, center, cv::Scalar(0, 0, 255), cv::MARKER_CROSS);
            cv::circle(img_update, startpt, 3, cv::Scalar(255, 255, 0), -1);
            cv::circle(img_update, endpt, 3, cv::Scalar(255, 0, 0), -1); 
            
            if (ceil(distance) <= 25.0) 
            {
                cv::line(img_update, startpt, endpt, CV_RGB(0, 255, 0), 2);
                std::cout <<"Line: (" <<startpt.x << ", " << startpt.y << ") - ("<<endpt.x << ", " << endpt.y 
                <<") distance: " << distance << std::endl; 
                temp.virtual_wll = std::make_pair(startpt, endpt);
                temp.distance = distance;
                result.emplace_back(temp);
            }
        } 
        cv::imshow("test", img_update);
        cv::waitKey(0); 
    }
    ...
    {
        cv::Point startpt = result[i].virtual_wll.first;
        cv::Point endpt = result[i].virtual_wll.second;
        double distance = result[i].distance;
    ...
    std::cout <<"==============================================================" <<std::endl;
    // Remove duplicates
    std::vector<LINEINFO> uniqueLines = removeDuplicates(result);
    ...
    cv::Mat img_test = result_img.clone();
    ...
    for (const auto& line : filteredLines) {
        ...
        cv::line(img_test, line.virtual_wll.first, line.virtual_wll.second, CV_RGB(0, 255, 0), 2);
    }
    cv::namedWindow("img_test", WINDOW_KEEPRATIO && WINDOW_AUTOSIZE);
